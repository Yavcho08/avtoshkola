...
exports.sendLessonCreatedEmails = sendLessonCreatedEmails;
exports.sendLessonCancelledEmails = sendLessonCancelledEmails;
exports.sendLessonCompletedEmail = sendLessonCompletedEmail;
...
function layout(title, accentColor, body) {
    return `<!DOCTYPE html>
<html lang="bg">
<head>
...
// ── Lesson created ────────────────────────────────────────────────────────────
async function sendLessonCreatedEmails(lesson) {
    ...
    if (studentEmail) {
        sends.push(getResend().emails.send({
            ...
            html: layout('Насрочено занятие', '#1d4ed8', `${greeting(lesson.students?.profiles?.first_name ?? '')}
    ...
    if (instructorEmail) {
        sends.push(getResend().emails.send({
            ...
            html: layout('Насрочено занятие', '#1d4ed8', `${greeting(lesson.instructors?.profiles?.first_name ?? '')}
         <p style="margin:12px 0 0;font-size:14px;font-weight:600;color:#374151;">
...
// ── Lesson cancelled ──────────────────────────────────────────────────────────
async function sendLessonCancelledEmails(lesson) {
    const typeLabel = lesson.type === 'theory' ? 'Теория' : 'Практика';
    const dt = fmtDT(lesson.start_time);
    const studentName = `${lesson.students?.profiles?.first_name ?? ''} ${lesson.students?.profiles?.last_name ?? ''}`.trim();
    ...
    if (studentEmail) {
        sends.push(getResend().emails.send({
            ...
            html: layout('Отказано занятие', '#b91c1c', `${greeting(lesson.students?.profiles?.first_name ?? '')}
         <p style="margin:12px 0 0;font-size:14px;font-weight:600;color:#374151;">
    ...
    if (instructorEmail) {
        sends.push(getResend().emails.send({
            ...
            html: layout('Отказано занятие', '#b91c1c', `${greeting(lesson.instructors?.profiles?.first_name ?? '')}
         <p style="margin:12px 0 0;font-size:14px;font-weight:600;color:#374151;">
...
// ── Lesson completed ──────────────────────────────────────────────────────────
async function sendLessonCompletedEmail(lesson) {
    ...
    await getResend().emails.send({
        ...
        html: layout('Завършено занятие', '#15803d', `${greeting(lesson.students?.profiles?.first_name ?? '')}
       <p style="margin:12px 0 0;font-size:14px;font-weight:600;color:#374151;">
