...
exports.studentStats = exports.instructorStats = exports.adminStats = void 0;
...
const adminStats = async (_req, res) => {
    ...
    const [totalStudents, activeStudents, activeInstructors, activeVehicles, revenueRes, expensesRes, upcomingExams, overduePayments,] = await Promise.all([
        supabase_1.supabase.from('students').select('id', { count: 'exact', head: true }),
        supabase_1.supabase.from('students').select('id', { count: 'exact', head: true }).eq('status', 'active'),
        supabase_1.supabase.from('instructors').select('id', { count: 'exact', head: true }).eq('is_active', true),
        supabase_1.supabase.from('vehicles').select('id', { count: 'exact', head: true }).eq('status', 'active'),
        ...
        supabase_1.supabase.from('exams').select('id', { count: 'exact', head: true })
            .eq('status', 'scheduled').gte('exam_date', todayIso),
        supabase_1.supabase.from('payments').select('id', { count: 'exact', head: true }).eq('status', 'overdue'),
    ]);
    if (revenueRes.error) {
        (0, response_1.sendError)(res, revenueRes.error.message, 500);
        return;
    }
...
const instructorStats = async (req, res) => {
    const user = req.user;
    ...
    const [upcoming, completedThisMonth, assignedStudents] = await Promise.all([
        supabase_1.supabase.from('lessons').select('id', { count: 'exact', head: true })
        ...
        supabase_1.supabase.from('student_categories').select('student_id', { count: 'exact', head: true })
            .eq('instructor_id', instructor.id),
    ]);
    // Next 5 upcoming lessons with student info
    const { data: nextLessons } = await supabase_1.supabase
        .from('lessons')
...
const studentStats = async (req, res) => {
    ...
    const [lessonsRes, examsRes, paymentsRes, nextLessons, nextExams] = await Promise.all([
        supabase_1.supabase.from('lessons').select('type, start_time, end_time').eq('student_id', student.id).eq('status', 'completed'),
        supabase_1.supabase.from('exams').select('status').eq('student_id', student.id),
        supabase_1.supabase.from('payments').select('amount, status').eq('student_id', student.id),
        supabase_1.supabase.from('lessons')
        ...
        supabase_1.supabase.from('exams')
            .select('*')
            .eq('student_id', student.id).eq('status', 'scheduled')
    ...
    const calcHours = (ls) => +ls.reduce((sum, l) => {
        return sum + (new Date(l.end_time).getTime() - new Date(l.start_time).getTime()) / 3600000;
    }, 0).toFixed(1);
    (0, response_1.sendSuccess)(res, {
        studentStatus: student.status,
        completedTheoryHours: calcHours(lessons.filter(l => l.type === 'theory')),
        completedPracticeHours: calcHours(lessons.filter(l => l.type === 'practice')),
...
exports.studentStats = studentStats;
