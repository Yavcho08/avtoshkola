...
async function getAuthEmail(profileId: string): Promise<string | null> {
...
export async function sendLessonReminders(): Promise<void> {
  ...
  for (const lesson of lessons) {
    const studentRec    = (lesson as any).students;
    const instructorRec = (lesson as any).instructors;
    ...
    if (studentEmail) {
      try {
        ...
        console.error(`[Reminder] ✗ Student → ${studentEmail}:`, (err as Error).message);
      }
    }
    ...
    if (instructorEmail) {
      try {
        ...
        console.error(`[Reminder] ✗ Instructor → ${instructorEmail}:`, (err as Error).message);
      }
    }
  }
